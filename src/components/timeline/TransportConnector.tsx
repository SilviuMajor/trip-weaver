...
const TransportConnector = ({
  ...
  return (
    ...
    >
      {/* Layer 1: Background strip — match card column width */}
      <div
        className="absolute top-0 bottom-0 rounded-[4px] overflow-hidden"
        style={{ backgroundColor: stripColor, left: 0, right: 4 }}
      />

      {/* Layer 2: Pill + from→to label — LEFT-aligned together */}
      <div className="absolute left-1 top-1/2 -translate-y-1/2 z-20 pointer-events-auto flex flex-col items-start gap-0.5">
        {expanded ? (
          /* ─── Expanded state ─── */
          <div
            className="rounded-3xl flex items-center gap-0.5 px-1.5 py-0.5"
            style={{
        ...
        {/* From→To label — directly below the pill, never overlapping */}
